...
$apiProc = Start-Process -FilePath "python" -ArgumentList "-m", "uvicorn", "app.main:app", "--reload", "--port", "8000" -WorkingDirectory $apiDir -PassThru -WindowStyle Minimized
Write-Host "  API started (PID: $($apiProc.Id)) on http://localhost:8000" -ForegroundColor Green
Pop-Location
...
$webProc = Start-Process -FilePath "node" -ArgumentList ".\node_modules\next\dist\bin\next", "dev", "--turbo" -WorkingDirectory $webDir -PassThru -WindowStyle Minimized
